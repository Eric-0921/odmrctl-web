...
use crate::types::{
    CommandAuditEntry, RecurMicrotestEvent, RecurMicrotestManifest, RecurMicrotestReport,
    RecurMicrotestSnapshot,
};
use serde::Serialize;
use std::fs;
use std::io::Write;
...
pub fn write_artifacts(
    out_dir: &Path,
    manifest: &RecurMicrotestManifest,
    snapshot: &RecurMicrotestSnapshot,
    report: &RecurMicrotestReport,
    events: &[RecurMicrotestEvent],
    audit: &[CommandAuditEntry],
) -> Result<(), String> {
    fs::create_dir_all(out_dir).map_err(|e| format!("create out dir: {e}"))?;

    write_json(out_dir, "manifest.json", manifest)?;
    write_json(out_dir, "maynuo_recur_microtest_snapshot.json", snapshot)?;
    write_json(out_dir, "maynuo_recur_microtest_report.json", report)?;
    write_jsonl(out_dir, "maynuo_recur_microtest_events.jsonl", events)?;
