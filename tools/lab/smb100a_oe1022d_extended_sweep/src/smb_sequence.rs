//! SMB100A software-stepped sweep sequence — step-level primitives (M3.3 extended).
//!
...
// ---------------------------------------------------------------------------
// Safety limits (M3.3 — bumped from M3.2)
// ---------------------------------------------------------------------------

const HARD_MAX_RF_POWER_DBM: f64 = -10.0;
const HARD_MAX_POINTS: u64 = 21;
const HARD_MAX_FRAMES_PER_STEP: u64 = 10;
const HARD_MAX_REPEAT_COUNT: u64 = 3;
const HARD_MAX_TOTAL_FRAMES: u64 = 630;
...
pub fn validate_safety_limits(cli: &Cli) -> Result<(), String> {
    ...
    if cli.repeat_count > HARD_MAX_REPEAT_COUNT {
        errors.push(format!(
            "repeat_count {} exceeds hard limit of {}",
            cli.repeat_count, HARD_MAX_REPEAT_COUNT
        ));
    }
    let total_frames = cli.rf_points * cli.frames_per_step * cli.repeat_count;
    if total_frames > HARD_MAX_TOTAL_FRAMES {
        errors.push(format!(
            "total frames {} ({}×{}×{}) exceeds hard limit of {}",
            total_frames, cli.repeat_count, cli.frames_per_step, cli.rf_points,
...
pub fn run_preflight(
    ...
    let operator_approval_present = cli.operator_approves_extended_sweep;
    let points_within = cli.rf_points <= HARD_MAX_POINTS;
    let repeat_within = cli.repeat_count <= HARD_MAX_REPEAT_COUNT;
    let total_ok = cli.rf_points * cli.frames_per_step * cli.repeat_count <= HARD_MAX_TOTAL_FRAMES;
    let passed = outp_off && mod_off && error_queue_clean && operator_approval_present
        && points_within && repeat_within && total_ok;
    ...
    let preflight = PreflightCheck {
        ...
        points_within_limit: points_within,
        repeat_within_limit: repeat_within,
...
pub fn configure_smb_common(
    ...
    if cli.set_internal_lf {
        ...
        if lfo_resp.trim() == "1" || lfo_resp.trim().eq_ignore_ascii_case("ON") {
            return Err("LFO output is ON; must remain OFF in M3.3".into());
        }
        let _ = (lf_freq_resp, lf_shape_resp, lf_volt_resp);
    }
