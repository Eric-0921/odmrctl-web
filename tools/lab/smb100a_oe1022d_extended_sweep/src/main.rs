//! M3.3: Extended SMB100A Software-Stepped Sweep + OE1022D Passive Acquisition.
//!
//! Extends M3.2 with repeat sweeps, parse failure quarantine, and
//! step-level B-channel statistics. First multi-repeat two-device test.
//!
//! This is NOT a full ODMR experiment. NOT an internal sweep. NOT GUI.
//!
//! ## Safety
//! - FM:STAT ON / MOD:STAT ON / OUTP ON require `--operator-approves-extended-sweep`
//! - Pre-flight must confirm OUTP=OFF, MOD:STAT=OFF, SYST:ERR clean
//! - Power, FM deviation, points, repeat, frames-per-step have hard limits
...
pub use statistics::{aggregate_b_channel_stats, compute_vector_stats, BChannelVectorStats};
...
fn main() {
    ...
    if let Err(e) = app::run_app(&cli) {
        eprintln!("M3.3 extended sweep failed: {}", e);
        std::process::exit(1);
    }
}
